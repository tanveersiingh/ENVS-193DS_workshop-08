...
plot(model6)
plot(model7)
# all residuals look nromal and no outliers
```

## Model selection

```{r model-selection}
AICc(model1, model2, model3, model4, model5, model6, model7, model0) |>
  arrange(AICc)
```
...
The best model that predicts total plan pass (g) includes water treatment and species (F(7,62)=27.08, p < 0.001, $\alpha$ = 0.05), but not SLA.
